...
export default function Buy({ itemID }) {
    ...
    const [status, setStatus] = useState(STATUS.Initial); // Tx status
    ...
    useEffect(() => {
        ...
        // If yes, fetch the item and set paid to true
    ...
    if (!publicKey) {
        return (
            <div>
                <p>You need to connect your wallet to make tranasactions!</p>
            </div>
        )
    }
    ...
    return (
        <div>
            {status === STATUS.Paid ? (
                <IPFSDownload filename="emojis.zip" hash="QmSr4PmwpnDfLVmMH4nEqvsooivtSgtBZHHTdubigFWsHX" cta="Download emojis"/>
            ) : (
